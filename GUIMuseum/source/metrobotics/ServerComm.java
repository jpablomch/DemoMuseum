...
public class ServerComm {
	...
	public void check4Msg() {
		...
		try {
			if(dataIn.available()>0){
				...
				else if((messageToken.compareTo("MOVING")) == 0){
					movingProc(msgScan);
				}
				else {
	...
	public void movingProc(Scanner msgScan){
		mf.grid.pathCalc.clear();
		int size = Integer.parseInt(msgScan.next());
		for(int i=0;i<size; i++){
			Point newPoint = new Point();
			newPoint.setLocation(Integer.parseInt(msgScan.next()), 
					Integer.parseInt(msgScan.next()));
			mf.grid.pathCalc.add(newPoint);
		}
		mf.grid.repaint();
	}
...
//				bot.setGridConfidence(conf);
		...
		if(repaint){
			...
			if(Gui.layoutSelection == GUIConstants.LAYOUT_ORIGINAL || Gui.layoutSelection == GUIConstants.LAYOUT_VIDEOANDCONTROL){
