...
public class Grid extends JPanel implements Scrollable, MouseListener, MouseMotionListener {
	...
	volatile ArrayList<Point> pathCalc;
...
//        g2.setStroke(new BasicStroke((float) 0.5));
        ...
        if(drawClick){
        	if(Robot.getRobotInUse() != -1){
            	...
            	if(!pathCalc.isEmpty()){
            		g2.drawLine((int)robots.get(Robot.getRobotInUse()).getGridX(), 
            				(int)robots.get(Robot.getRobotInUse()).getGridY(), 
            				(int)pathCalc.get(0).getX(), 
            				(int)pathCalc.get(0).getY());
            		int i; 
            		for(i=1; i<pathCalc.size(); i++){
            			 g2.drawLine((int)pathCalc.get(i-1).getX(), 
                 				(int)pathCalc.get(i-1).getY(), 
                 				(int)pathCalc.get(i).getX(), 
                				(int)pathCalc.get(i).getY());
            		 }
            		 g2.drawLine((int)pathCalc.get(i).getX(), 
             				(int)pathCalc.get(i).getY(),
             				(int)clicked.getX(),
             				(int)clicked.getY());
            	}
...
//		}
    	...
    	for (Robot x : robots){
			...
			String name = x.getName() + " (" + (int)x.getGridX() + ", " + (int)x.getGridY() + ", " + Math.toDegrees(x.getGridTheta()) + " c:" + df.format(x.getConfidence());
			if(name!=null){
