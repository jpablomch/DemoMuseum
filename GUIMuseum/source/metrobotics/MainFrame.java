 ...
 * RTEAM
 * Dic/2010
 * Demo for the Museum of Natural History
 *
 */
...
//	volatile boolean moving;
	...
	MainFrame(String title, ArrayList<Robot> robots){
		super(title);
		Dimension d = new Dimension(1000, 800); //750 was working fine. make it bigger on 09/18/2010 to fit the goto panel;
	    ...
	    if(Gui.layoutSelection == GUIConstants.LAYOUT_VIDEOANDCONTROL){
	    	...
	    	visionAndControlLayout();
	    }
	    
	    // CLEAN THIS!!!
	    // Old Layout 
	    else if(Gui.layoutSelection == GUIConstants.LAYOUT_ORIGINAL){
		    metroTitle = new Title();
		    RobotInUseLabel = new JLabel();
			vision = new VisionDisplay(robots, 400, 350, this); // 450, 350
			robotSel = new RobotSelector(RobotInUseLabel, robots, vision);
			...
			metroTitle = new Title();
			RobotInUseLabel = new JLabel();
			vision = new VisionDisplay(robots, 400, 350, this);
			robotSel = new RobotSelector(RobotInUseLabel, robots, vision);
			behaviors = new Behaviors(robots);
...
//	}
	...
	private void visionAndControlLayout(){
...
//		}
	...
	public void newRobotSelAndGrid(){
		...
		robotSel = new RobotSelector(RobotInUseLabel, robots, vision);
		System.out.println("new RobotSel created ");
		scrollPane = new JScrollPane(robotSel);
		if(Gui.layoutSelection == GUIConstants.LAYOUT_VIDEOANDCONTROL){
			scrollPane.setPreferredSize(new Dimension(150, 600));			
		}
